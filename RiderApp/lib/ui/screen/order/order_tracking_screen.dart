...
class OrderTrackingScreen extends StatefulWidget {
  final String? id, customerAddress, restaurantAddress, customerName, customerMobile, customerImage;
  final double? customerLatitude, customerLongitude, restaurantLatitude, restaurantLongitude;
    ...
    return CupertinoPageRoute(
        builder: (_) => BlocProvider<OrderCubit>(
              ...
              child: OrderTrackingScreen(
                  ...
                  restaurantLongitude: arguments['restaurantLongitude'] as double,
...
class _OrderTrackingScreenState extends State<OrderTrackingScreen> with SingleTickerProviderStateMixin {
  ...
  _launchMap() async {
    ...
    if (widget.isTracking!) {
      _addMarker(LatLng(widget.customerLatitude!, widget.customerLongitude!), "destination");
    }else{
      _addMarker(LatLng(widget.restaurantLatitude!, widget.restaurantLongitude!), "destination");
    }
    timer = Timer.periodic(const Duration(seconds: 2), (Timer t) async {
      ...
      if (widget.isTracking!) {
        context.read<ManageLiveTrackingCubit>().manageLiveTracking(
            orderId: widget.id, orderStatus: outForDeliveryKey, latitude: _originLatitude.toString(), longitude: _originLongitude.toString());
      }
      updateMarker(
    ...
    return _connectionStatus == connectivityCheck
        ...
        : PopScope(canPop: true,
            onPopInvoked: (value) async {
              context.read<SettingsCubit>().setLatitude(_originLatitude.toString());
              context.read<SettingsCubit>().setLongitude(_originLongitude.toString());
              //return true;
            },
            child: Scaffold(
                appBar: DesignConfig.appBar(
                  ...
                  from: "map",
                  latitude: context.read<SettingsCubit>().setLatitude(_originLatitude.toString()),
                  longitude: context.read<SettingsCubit>().setLongitude(_originLongitude.toString()),
                ),
                body: BlocListener<ManageLiveTrackingCubit, ManageLiveTrackingState>(
                                ...
                                child: Stack(
                                  children: [
                                            ...
                                            children: [
                                              CircleAvatar(
                                                ...
                                                child: CircleAvatar(
                                                  radius: 18,
                                                  backgroundColor: Theme.of(context).colorScheme.onSurface,
                                                  child: DesignConfig.imageWidgets(orderList[0].profile, 85, 85, "2"),
                                                ),
                                              ),
                                              Padding(
                                          ...
                                          GestureDetector(
                                            ...
                                            child: Container(
                                              ...
                                              decoration: DesignConfig.boxDecorationContainer(
                                                  Theme.of(context).colorScheme.onSurface.withOpacity(0.30), 4.0),
                                    ...
                                    Container(
                                      ...
                                      margin: EdgeInsetsDirectional.only(top: height! / 1.8),
                                          ...
                                          children: [
                                            Column(
                                                ...
                                                children: [
                                                  Padding(
                                                    padding:
                                                        EdgeInsetsDirectional.only(top: height! / 40.0, start: width! / 40.00, end: width! / 40.0),
                                                    child: Row(
                                                      children: [
                                                        Container(
  ...
  _addMarker(LatLng position, String id) async {
    ...
    if (widget.isTracking!) {
      if (id == "destination") {
        restaurantsIcon = await bitmapDescriptorFromSvgAsset(context, DesignConfig.setSvgPath("address_icons"));
        defaultIcon = BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueYellow);
        icon = restaurantsIcon;
      }
    } else {
      if (id == "destination") {
        destinationIcon = await bitmapDescriptorFromSvgAsset(context, DesignConfig.setSvgPath("map_pin"));
  ...
  _getPolyline() async {
    ...
    if (widget.isTracking!) {
      mainroute =
          await getRouteBetweenCoordinates(LatLng(_originLatitude!, _originLongitude!), LatLng(widget.customerLatitude!, widget.customerLongitude!));
    } else {
      mainroute = await getRouteBetweenCoordinates(
          LatLng(_originLatitude!, _originLongitude!), LatLng(widget.restaurantLatitude!, widget.restaurantLongitude!));
    }
    ...
    if (mainroute.isEmpty) {
      ...
      if (widget.isTracking!) {
        mainroute.add(LatLng(widget.customerLatitude!, widget.customerLongitude!));
      } else {
        mainroute.add(LatLng(widget.restaurantLatitude!, widget.restaurantLongitude!));
      }
    }
