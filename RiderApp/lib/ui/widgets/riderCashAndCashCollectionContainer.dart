...
class RiderCashAndCashCollectionContainer extends StatelessWidget {
  final CashCollectionModel cashCollectionModel;
  final double? width, height;
  final int? index;
  const RiderCashAndCashCollectionContainer({Key? key, required this.cashCollectionModel, this.width, this.height, this.index}) : super(key: key);
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.start, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  Text(" #${cashCollectionModel.id!}",
                      textAlign: TextAlign.start,
                      style: TextStyle(color: Theme.of(context).colorScheme.secondary, fontWeight: FontWeight.w700, fontSize: 14.0)),
                ],
              ),
              cashCollectionModel.type == ""
                  ? const SizedBox()
                  : Align(
                      ...
                      child: Container(
                        ...
                        decoration: DesignConfig.boxDecorationContainerBorder(
                            DesignConfig.riderCashStatusCartColor(cashCollectionModel.type!.toLowerCase()),
                            DesignConfig.riderCashStatusCartColor(cashCollectionModel.type!.toLowerCase()).withOpacity(0.10),
                            4.0),
                        child: Text(
                          cashCollectionModel.type!,
                          style: TextStyle(
                              ...
                              color: DesignConfig.riderCashStatusCartColor(cashCollectionModel.type!.toLowerCase())),
                        ),
                      ),
                    ),
            ],
          ...
          Text(formatter.format(DateTime.parse(cashCollectionModel.transactionDate!)),
              textAlign: TextAlign.start,
          ...
          SizedBox(
              width: width! / 1.1,
              child: Text(cashCollectionModel.message!,
                  textAlign: TextAlign.start,
                  style: TextStyle(color: Theme.of(context).colorScheme.onPrimary.withOpacity(0.76), fontWeight: FontWeight.w600, fontSize: 14.0),
                  maxLines: 2,
          ...
          Row(children: [
            ...
            const Spacer(),
            Text("${context.read<SystemConfigCubit>().getCurrency()}${double.parse(cashCollectionModel.amount!).toStringAsFixed(2)}",
                textAlign: TextAlign.start,
                style: TextStyle(
