...
class BottomSheetEditItemContainer extends StatefulWidget {
  ...
  BottomSheetEditItemContainer({Key? key, required this.productDetailsModel, this.width, this.height}) : super(key: key);

  @override
  State<BottomSheetEditItemContainer> createState() => _BottomSheetEditItemContainerState();
}

class _BottomSheetEditItemContainerState extends State<BottomSheetEditItemContainer> {
  ...
  void initState() {
    if (context.read<AuthCubit>().state is AuthInitial || context.read<AuthCubit>().state is Unauthenticated) {
      context.read<GetOfflineCartCubit>().getOfflineCart(widget.productDetailsModel, context);
    } else {
      cartList = context.read<GetCartCubit>().getCartProductList(widget.productDetailsModel.id!);
    }
  ...
  Widget offlineCartWidget() {
    return BlocConsumer<GetOfflineCartCubit, GetOfflineCartState>(
      ...
      builder: (context, state) {
        ...
        final offlineCartList = (state as GetOfflineCartSuccess).offlineCartList;
        return offlineCartList.isEmpty?const SizedBox.shrink():ListView.builder(
            shrinkWrap: true,
              ...
              return BlocBuilder<GetOfflineCartCubit, GetOfflineCartState>(
                builder: (context, state) {
                  return Builder(builder: (context) {
                    return Container(
                      margin: EdgeInsetsDirectional.only(start: widget.width! / 20.0, end: widget.width! / 20.0),
                      padding: EdgeInsetsDirectional.only(bottom: widget.height! / 99.0),
                      width: widget.width!,
                      child: Column(
                        ...
                        children: [
                          Padding(
                            padding: EdgeInsetsDirectional.only(start: widget.width! / 50.0, bottom: widget.height! / 99.0),
                            child: Column(mainAxisAlignment: MainAxisAlignment.start, crossAxisAlignment: CrossAxisAlignment.start, children: [
                              Column(
                                children: List.generate(widget.productDetailsModel.variants!.length, (l) {
                                  VariantsModel variantData = widget.productDetailsModel.variants![l];
                                  double price = double.parse(variantData.specialPrice!);
                                  if (price == 0) {
                                    price = double.parse(variantData.price!);
                                  }
                                  double off = 0;
                                  if (widget.productDetailsModel.variants![l].specialPrice! != "0") {
                                  ...
                                  var sum = 0.0;
                                  for (var h = 0; h < widget.productDetailsModel.productAddOns!.length; h++) {
                                    if (offlineCartList[i].addOnId!.contains(widget.productDetailsModel.productAddOns![h].id!)) {
                                      sum += double.parse(widget.productDetailsModel.productAddOns![h].price!) *
                                          int.parse(offlineCartList[i].qty!);
                                    }
                                  }
                                  double overAllTotal = ((price * int.parse(offlineCartList[i].qty!)) + sum);
                                  return (offlineCartList[i].vId == widget.productDetailsModel.variants![l].id!)
                                      ? Column(
                                          mainAxisAlignment: MainAxisAlignment.start,
                                          crossAxisAlignment: CrossAxisAlignment.start,
                                          children: [
                                                  ...
                                                  children: [
                                                    ...
                                                    Expanded(
                                                      ...
                                                      child: SizedBox(
                                                        ...
                                                        child: Text(
                                                          widget.productDetailsModel.name!,
                                                          textAlign:
                                                            ...
                                                            InkWell(
                                                              ...
                                                              onTap: () async {
                                                                ...
                                                                List<String> productAddons =
                                                                    offlineCartList[i].addOnId!.split(',').map((str) => str.trim()).toList();
                                                                ...
                                                                for (int qt = 0; qt < addOnsDataModel.length; qt++) {
                                                                  if (productAddons.contains(addOnsDataModel[qt].id)) {
                                                                    ...
                                                                    addOnQty
                                                                        .add((int.parse(offlineCartList[i].qty.toString()) - 1).toString());
                                                                    totalSum += (double.parse(addOnsDataModel[qt].price!.toString()) *
                                                                        (int.parse(offlineCartList[i].qty.toString()) - 1));
                                                                  }
                                                                }
                                                                double overAllTotalPrice = (price * (int.parse(offlineCartList[i].qty.toString()) - 1) + totalSum);
        
                                                                if (int.parse(offlineCartList[i].qty.toString()) == 1) {
                                                                  db.removeCart(variantData.id!, widget.productDetailsModel.id!, context,
                                                                  ...
                                                                  db.insertCart(
                                                                      ...
                                                                      context,
                                                                      edit: true,
                                                                      id: int.parse(offlineCartList[i].id!))
                                                                    .whenComplete(() async {
                                                                      context.read<OfflineCartCubit>().updateQuntity(widget.productDetailsModel,
                                                                            ((int.parse(offlineCartList[i].qty.toString()) - 1)).toString(),variantData.id);
                                                                      offlineCartList[i].qty = (int.parse(offlineCartList[i].qty!) - 1).toString();
                                                                      //context.read<GetOfflineCartCubit>().updateQuntity(offlineCartList[i], (int.parse(offlineCartList[i].qty.toString()) - 1).toString(), i);
                                                                      setState(() {});
                                                                      });
                                                                }
                                                                setState(() {});
                                                              ...
                                                              child:
                                                                  Icon(Icons.remove, color: Theme.of(context).colorScheme.onPrimary, size: 15.0),
                                                            ),
                                                            const Spacer(),
                                                            offlineCartList.isNotEmpty
                                                                ? Text(
                                                                    offlineCartList[i]
                                                                        .qty! /* context.read<GetOfflineCartQuantityCubit>().fetchQty()[i].qty */,
                                                            ...
                                                            InkWell(
                                                              ...
                                                              onTap: () {
                                                                ...
                                                                double overAllTotalPrice =
                                                                    (price * (int.parse(offlineCartList[i].qty.toString()) + 1) + totalSum);
                                                                setState(() {
                                                                  if (int.parse(offlineCartList[i].qty.toString()) <
                                                                    ...
                                                                    UiUtils.setSnackBar(
                                                                      ...
                                                                      int.parse(offlineCartList[i].qty.toString()) >=
                                                                          int.parse(widget.productDetailsModel.totalAllowedQuantity!)) {
                                                                    UiUtils.setSnackBar(
                                                ...
                                                child: Row(
                                                  ...
                                                  children: [
                                                    ...
                                                    Row(
                                                      mainAxisSize: MainAxisSize.min,
                                                      mainAxisAlignment: MainAxisAlignment.start,
                                                      children: List.generate(widget.productDetailsModel.productAddOns!.length, (m) {
                                                        ProductAddOnsModel addOnData = widget.productDetailsModel.productAddOns![m];
                                                        return offlineCartList.isNotEmpty &&
                                                                offlineCartList[i].addOnId!.contains(addOnData.id!)
                                                            ? Text(
                                                                "${addOnData.title!}, ",
                                                                textAlign: TextAlign.center,
                                                                style: TextStyle(
                ...
                child: Column(
                  ...
                  children: [
                    Padding(
                      padding: EdgeInsetsDirectional.only(start: widget.width! / 50.0, bottom: widget.height! / 99.0),
                      child: Column(
                        children: List.generate(cartList[i].productDetails!.length, (j) {
                          return Column(mainAxisAlignment: MainAxisAlignment.start, crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Column(
                              children: List.generate(cartList[i].productDetails![j].variants!.length, (l) {
                                VariantsModel variantData = cartList[i].productDetails![j].variants![l];
                                double price = double.parse(variantData.specialPrice!);
                                if (price == 0) {
                                  price = double.parse(variantData.price!);
                                }
                                ...
                                var sum = 0.0;
                                if (cartList[i].productDetails![j].variants![l].addOnsData!.isNotEmpty) {
                                  for (var k = 0; k < cartList[i].productDetails![j].variants![l].addOnsData!.length; k++) {
                                    sum += double.parse(cartList[i].productDetails![j].variants![l].addOnsData![k].price!.toString());
                                  }
                                }
                                return (cartList[i].productVariantId == cartList[i].productDetails![j].variants![l].id!)
                                    ? Column(mainAxisAlignment: MainAxisAlignment.start, crossAxisAlignment: CrossAxisAlignment.start, children: [
                                        Row(mainAxisAlignment: MainAxisAlignment.start, crossAxisAlignment: CrossAxisAlignment.start, children: [
                                          cartList[i].productDetails![j].indicator == "1"
                                              ? SvgPicture.asset(DesignConfig.setSvgPath("veg_icon"), width: 15, height: 15)
                                              : cartList[i].productDetails![j].indicator == "2"
                                                  ? SvgPicture.asset(DesignConfig.setSvgPath("non_veg_icon"), width: 15, height: 15)
                                                  : const SizedBox(height: 15, width: 15.0),
                                          const SizedBox(width: 5.0),
                                          Expanded(
                                                                  ...
                                                                  setState(() {
                                                                    if (int.parse(cartList[i].qty!) <= int.parse(cartList[i].minimumOrderQuantity!)) {
                                                                      context.read<RemoveFromCartCubit>().removeFromCart(
                                                                          /* userId: context.read<AuthCubit>().getId(), */
                                                                          cartId: cartList[i].productVariantId,
                                                                          branchId: context.read<SettingsCubit>().getSettings().branchId);
                                                            ...
                                                            setState(() {
                                                              if (int.parse(cartList[i].qty!) <
                                                                ...
                                                                UiUtils.setSnackBar(
                                                                    ...
                                                                    false,
                                                                    type: "2");
                                                              } else if (cartList[i].productDetails![j].totalAllowedQuantity != "" &&
                                                                  int.parse(cartList[i].qty!) >=
                                                                      int.parse(cartList[i].productDetails![j].totalAllowedQuantity!)) {
                                                                cartList[i].qty = cartList[i].productDetails![j].totalAllowedQuantity!;
                                                                UiUtils.setSnackBar(
                                            ...
                                            children: [
                                              variantData.variantValues != ""
                                                  ? Text(
                                                      ...
                                                      textAlign: TextAlign.left,
                                                      style: TextStyle(
                                                          color: Theme.of(context).colorScheme.secondary, fontSize: 10, fontWeight: FontWeight.w600),
                                                    )
                                                  : Container(height: 10.0),
                                              cartList[i].productDetails![j].variants![l].addOnsData!.isNotEmpty
                                                  ? Row(
                                                      mainAxisSize: MainAxisSize.min,
                                                      mainAxisAlignment: MainAxisAlignment.start,
                                                      children: List.generate(cartList[i].productDetails![j].variants![l].addOnsData!.length, (m) {
                                                ...
                                                child: Row(
                                                  children: [
                                                    Text(
                                                      UiUtils.getTranslatedLabel(context, editLabel),
                                                      style: TextStyle(
                                                          fontSize: 12, color: Theme.of(context).colorScheme.secondary, fontWeight: FontWeight.w500),
                                                    ),
                                                    Icon(Icons.edit_outlined, color: Theme.of(context).colorScheme.secondary, size: 12.0),
                                                  ],
                                                ),
                                                ...
                                                children: [
                                                  Text(
                                                    context.read<SystemConfigCubit>().getCurrency() +
                                                        (double.parse(price.toString()) * int.parse(cartList[i].qty!) +
                                                                (sum * int.parse(cartList[i].qty!)))
                                                            .toStringAsFixed(2),
                                                    textAlign: TextAlign.center,
                                                    style: TextStyle(
