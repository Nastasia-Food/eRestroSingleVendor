...
class SearchBarContainer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        padding: EdgeInsetsDirectional.only(start: width! / 35.0),
        child: Text(title!,
...
class _SearchBarContainerState extends State<SearchBarContainer> with TickerProviderStateMixin {
  late final AnimationController _animationController = AnimationController(vsync: this, duration: const Duration(milliseconds: 2500));

  late final Animation<double> _bottomToCenterTextAnimation =
      Tween<double>(begin: 1, end: 0).animate(CurvedAnimation(parent: _animationController, curve: const Interval(0.0, 0.25)));

  late final Animation<double> _centerToTopTextAnimation =
      Tween<double>(begin: 0, end: 1).animate(CurvedAnimation(parent: _animationController, curve: const Interval(0.75, 1.0)));
    ...
    return BlocListener<CuisineCubit, CuisineState>(
      ...
      child: AnimatedBuilder(
          ...
          builder: (context, child) {
            final dy = _bottomToCenterTextAnimation.value - _centerToTopTextAnimation.value;

            final opacity = (1 - _bottomToCenterTextAnimation.value - _centerToTopTextAnimation.value);

            return Container(
                height: widget.height! / 20.0,
                alignment: Alignment(-1.0, dy), //alignment: Alignment.centerLeft,
                decoration: DesignConfig.boxDecorationContainerBorder(textFieldBorder, textFieldBackground, 4.0),
                padding: EdgeInsetsDirectional.only(start: widget.width! / 35.0),
                child: Opacity(
                    opacity: opacity,
                    child: /* Text("${cuisineList.isNotEmpty?"Search":widget.title!} ${cuisineList.isNotEmpty?cuisineList[index].name:""}",
                      style:
                          TextStyle(color: Theme.of(context).colorScheme.onPrimary.withOpacity(0.76), fontSize: 16.0, fontWeight: FontWeight.w500)) */
                        RichText(
                      text: TextSpan(
                        text: cuisineList.isNotEmpty ? UiUtils.getTranslatedLabel(context, searchLabel) : widget.title!,
                        style: TextStyle(
                        ...
                        children: [
                          TextSpan(
                              text: " ${cuisineList.isNotEmpty ? cuisineList[index].name : ""}",
                              style: TextStyle(
                                  color: Theme.of(context).colorScheme.onPrimary.withOpacity(0.76),
                                  fontSize: 16.0,
