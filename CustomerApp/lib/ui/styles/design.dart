...
class DesignConfig {
  ...
  static BoxDecoration boxDecorationBorder(Color color, double radius, {bool status = false}) {
    return BoxDecoration(
      border: Border.all(color: color, width: status == false ? 1 : 0.5),
      borderRadius: BorderRadius.circular(radius),
    );
  ...
  static appBar(BuildContext context, double? width, String? text, bottom, {bool? status = false, double? preferSize = kToolbarHeight, bool? backButton = true}) {
    return PreferredSize(
      ...
      child: Container(
        ...
        child: AppBar(automaticallyImplyLeading: backButton!,leadingWidth: backButton?null:0.0,
          leading: backButton?InkWell(
              onTap: () {
                if (status == false) {
              ...
              child: Padding(
                  ...
                  child: Icon(Platform.isIOS ? Icons.arrow_back_ios : Icons.arrow_back, color: Theme.of(context).colorScheme.onPrimary))): const SizedBox.shrink(),
  ...
  static TextButton bottomBarTextButton(
      BuildContext context, int selectedIndex, int index, void Function() onPressed, String? activeIcon, String? inactiveIcon, String? iconTitle, EdgeInsetsGeometry? padding) {
    return TextButton(style: ButtonStyle(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            SvgPicture.asset(selectedIndex == index ? DesignConfig.setSvgPath(activeIcon!) : DesignConfig.setSvgPath(inactiveIcon!)),
            selectedIndex == index ? SizedBox(height: 2) : const SizedBox(),
            selectedIndex == index
                ? Text(iconTitle!.toUpperCase(),
                    style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Theme.of(context).colorScheme.onPrimary))
