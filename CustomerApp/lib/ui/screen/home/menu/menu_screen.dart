...
class MenuScreenState extends State<MenuScreen> with TickerProviderStateMixin {
  ...
  static void selectPage(BuildContext context, int index) {
    MenuScreenState? stateObject = context.findAncestorStateOfType<MenuScreenState>();
      ...
      child: _connectionStatus == connectivityCheck
          ...
          : BlocConsumer<CuisineCubit, CuisineState>(
              ...
              builder: (context, state) {
                if (state is CuisineProgress || state is CuisineInitial) {
                  return Scaffold(
                      backgroundColor: Theme.of(context).colorScheme.onSurface, body: MenuDetailSimmer(width: width!, height: height!));
                }
                if (state is CuisineFailure) {
                  return Scaffold(
                    backgroundColor: Theme.of(context).colorScheme.onSurface,
                    appBar: DesignConfig.appBar(context, width!, UiUtils.getTranslatedLabel(context, menuLabel),
                        const PreferredSize(preferredSize: Size.zero, child: SizedBox()), backButton: false),
                  );
                }
