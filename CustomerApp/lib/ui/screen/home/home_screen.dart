          ...
          if (mounted) {
            setState(() async {
              if (placemarks.first.subLocality == "" || placemarks.first.subLocality.toString().isEmpty) {
                currentAddress = "${placemarks.first.locality ?? placemarks.first.subAdminArea!}";
              } else {
              ...
              if (context.read<SystemConfigCubit>().getDemoMode() == "0") {
                ...
                setAddressForDisplayData(context, "0", placemarks.first.locality??placemarks.first.subAdminArea!, position.latitude.toString(),
              ...
              if (context.read<SettingsCubit>().state.settingsModel!.city.toString() != "" &&
                ...
                if (await Permission.location.serviceStatus.isEnabled) {
                  if (mounted) {
                    if (context.read<SystemConfigCubit>().getDemoMode() == "0") {
                      context.read<CityDeliverableCubit>().fetchCityDeliverable("Bhuj", "23.230141065546604", "69.6622062844058");
                    } else {
                      context
            ...
            children: [
              ...
              SizedBox(
                ...
                child: Text(
                  ...
                  style: TextStyle(color: Theme.of(context).colorScheme.onPrimary.withOpacity(0.76), fontSize: 12, fontWeight: FontWeight.w500, overflow: TextOverflow.ellipsis),
                  maxLines: 1,
                ),
              ),
            ]);
      ...
      child: GooglePlaceAutoCompleteTextField(
          ...
          getPlaceDetailWithLatLng: (p) async {
            ...
            if (mounted) {
              setState(() {
                ...
                /* if (searchAddressData.any((element) => element['city'].contains(infoAddress))) {
          ...
          itemClick: (p) async {
            ...
            if (mounted) {
              ...
              //print("infoAddress:==$infoAddress");
              /*setState(() {
                List<dynamic> localities = detail.result.addressComponents
                    .where((entry) => entry.types.contains('locality') || entry.types.contains('administrative_area_level_1') || entry.types.contains('administrative_area_level_2') return false;)
                    .toList()
                    .map((entry) => entry.longName)
                    .toList();
                if (context.read<SystemConfigCubit>().getDemoMode() == "0") {
    ...
    return BlocConsumer<OrderAgainCubit, OrderAgainState>(
        ...
        builder: (context, state) {
          ...
          if (state is OrderAgainFailure) {
            return (state.errorMessage.toString() == "No Order(s) Found !" || state.errorStatusCode.toString() == tockenExpireCode)
                ? Container()
                : Container();
          }
                              ...
                              child: Builder(builder: (context) {
                                return GestureDetector(
                                  ...
                                  child: Container(
                                      ...
                                      child: Container(
                                        decoration: DesignConfig.boxDecorationContainer(Theme.of(context).colorScheme.onSurface, 5.0),
                                        padding:
                                      ...
                                      : (index.isEven)
                                          ? SizedBox(
                                              ...
                                              child: SingleChildScrollView(
                                                ...
                                                child: Row(
                                                  children: List.generate(
                                                      sectionsList[index].productDetails!.length > 6 ? 6 : sectionsList[index].productDetails!.length, (i) {
                                          ...
                                          : SingleChildScrollView(
                                              ...
                                              child: Column(
                                                  children: List.generate(
                                                      sectionsList[index].productDetails!.length > 5 ? 5 : sectionsList[index].productDetails!.length, (i) {
                                                double price = double.parse(sectionsList[index].productDetails![i].variants![0].specialPrice!);
                                                if (price == 0) {
                                                  price = double.parse(sectionsList[index].productDetails![i].variants![0].price!);
                                                }
  ...
  Widget hotDeal() {
    return BlocConsumer<BestOfferCubit, BestOfferState>(
        ...
        builder: (context, state) {
          ...
          return bestOfferList.isNotEmpty?Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Padding(
