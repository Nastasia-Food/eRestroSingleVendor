...
import 'package:erestroSingleVender/ui/screen/payment/Stripe_Service.dart';
import 'package:erestroSingleVender/ui/screen/payment/midtransWebView.dart';
import 'package:erestroSingleVender/ui/screen/settings/no_internet_screen.dart';
import 'package:erestroSingleVender/ui/screen/payment/payment_radio.dart';
import 'package:erestroSingleVender/ui/screen/payment/paypal_webview_screen.dart';
...
import 'package:erestroSingleVender/ui/widgets/transactionContainer.dart';
...
class WalletScreenState extends State<WalletScreen> with SingleTickerProviderStateMixin {
  ...
  List<String> paymentIconList = ['paypal', 'rozerpay', 'paystack', 'flutterwave', 'stripe', 'paytm', 'midtrans', 'phonepe'];
  ...
  Future<void> sendRequest(String txnId, String payMethod, {String? OrderID, String? status, Map<String, dynamic>? jsonDatas, String? checksums}) async {
    ...
    try {
      ...
      if (payMethod.toLowerCase() == "stripe" || payMethod.toLowerCase() == "paypal"/*  || payMethod.toLowerCase() == "phonepe" */) {
        body[skipVerifyTransactionKey] = (payMethod.toLowerCase() == "stripe" || payMethod.toLowerCase() == "paypal"/*  || payMethod.toLowerCase() == "phonepe" */) ? "true" : "false";
      }
    ...
    try {
      ...
      try {
        ...
        await Api.post(body: body, url: Api.createMidtransTransactionUrl, token: true, errorCode: true).then(
          (result) {
            var getdata = result;//json.decode(result.body);
            bool error = getdata[errorKey];
                        ...
                        child: ListView(children: <Widget>[
                          Container(
                              ...
                              child: TextFormField(
                                ...
                                keyboardType: TextInputType.number,inputFormatters: [FilteringTextInputFormatter.allow(RegExp(r'^\d+\.?\d{0,2}')),],
  ...
  Future<void> phonepeCheckSum(double price) async {
    try {
      ...
      //var parameter = {orderIdKey: orderID, finalTotalKey: price.toStringAsFixed(2), mobileKey: context.read<AuthCubit>().getMobile()};
      var parameter = {
        typeKey: "wallet",
      ...
      print("payload:$phonePeMode--$checksum--$phonePeMerId--$appId");
  ...
  void initPhonePeSdk(double price, {required String orderId, Map<String, dynamic>? jsonDatas, String? checksums}) async {
    //phonePeMode = phonePeMode == "live" ? "PRODUCTION" : "SANDBOX";
    //String appId = "appId";
    print("phonePeMerId:$phonePeMerId--$phonePeMode");

    PhonePePaymentSdk.init(phonePeMode!, appId!, phonePeMerId!, true).then((isInitialized) {
      print("phonePeMerId:$phonePeMerId");
      sendRequest(/* uuid */ orderId, "phonepe", OrderID: orderId, status: waitingKey, jsonDatas: jsonDatas, checksums: checksums);
      //startPaymentPhonePe(price, orderId: orderId, jsonDatas: jsonDatas, checksums: checksums);
    }).catchError((error) {
      print("error::$error");
      // handleError(error);
      return error;
  ...
  void startPaymentPhonePe(double price, {required String orderId, Map<String, dynamic>? jsonDatas, String? checksums}) async {
    try {
      //var uuid = const Uuid().v4().substring(0, 30); // Generates a random UUID (v4)
      /* String amt = ((price) * 100).toStringAsFixed(2);
      ...
      PhonePePaymentSdk.startTransaction(body, phonePeEndPointUrl!, checksums!, /* pgHeaders, apiEndPoint, */
              /* "com.phonepe.app.preprod"*/ Platform.isAndroid ? packageName : iosPackage)
          .then((response) async {
        if (kDebugMode) {
        ...
        if (response != null) {
          ...
          if (status == 'SUCCESS') {
            ...
            //Map<String, dynamic> result = await updateOrderStatus(orderId: orderId, status: placedKey, uuid: uuid);
            print("data:--");
            //await sendRequest(/* uuid */ orderId, "phonepe", OrderID: orderId, status: waitingKey);
            context.read<SystemConfigCubit>().getSystemConfig(context.read<AuthCubit>().getId());
            context.read<TransactionCubit>().fetchTransaction(perPage, /* context.read<AuthCubit>().getId(),  */walletKey);
            /* if (mounted) {
            } */
                          ...
                          child: SingleChildScrollView(
                            ...
                            Container(
                                ...
                                child: TextFormField(
                                  ...
                                  validator: (val) {
                                      if(val!.isEmpty){
                                        return validateField(val, UiUtils.getTranslatedLabel(context, requirdFieldLabel));
                                      } else if(double.parse(withdrawAmountController!.text)> double.parse(context.read<SystemConfigCubit>().getWallet())){
                                        return UiUtils.getTranslatedLabel(context, notEnoughBalanceInWalletLabel);
                                      }
                                      return null;
                                    },
                                  autovalidateMode: AutovalidateMode.onUserInteraction,
                                ...
                                children: [
                                    ...
                                    child: Container(
                                      ...
                                      decoration: Directionality.of(context) == ui.TextDirection.rtl
                                          ? DesignConfig.boxDecorationContainerRoundHalf(
                                              Theme.of(context).colorScheme.onSurface.withOpacity(0.10), 0, 0, 10, 62)
                                          : DesignConfig.boxDecorationContainerRoundHalf(
                                  ...
                                  Positioned.directional(
                                    ...
                                    child: Container(
                                      ...
                                      decoration: Directionality.of(context) == ui.TextDirection.rtl
                                          ? DesignConfig.boxDecorationContainerRoundHalf(
                                              Theme.of(context).colorScheme.onSurface.withOpacity(0.10), 62, 10, 0, 0)
                                          : DesignConfig.boxDecorationContainerRoundHalf(
