...
class _AddressScreenState extends State<AddressScreen> {
  ...
  defaultLocation() async {
    ...
    setState(() {
      latlong = LatLng(
          double.parse(context.read<SettingsCubit>().getSettings().latitude), double.parse(context.read<SettingsCubit>().getSettings().longitude));
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      mobileNumberField(),
                      /* /* isPhoneValidated == "1" */ (phoneNumberController.text.isEmpty)
                          ? Padding(
                            padding: EdgeInsetsDirectional.only(start: width!/ 18.0),
                            child: Text("Please Enter Your Phone No",  style: TextStyle(fontSize: 12.0, color: Theme.of(context).colorScheme.error, fontWeight: FontWeight.w300)),
                                ...
                                return SizedBox(
                                  ...
                                  child: ButtonContainer(
                                    ...
                                    /* if(isPhoneValidated == ""){
                                      ...
                                      if (_formKey.currentState!.validate() && phoneNumberController.text.isNotEmpty) {
  ...
  Widget mobileNumberField() {
    return Container(
            ...
            child: IntlPhoneField(
              autovalidateMode: /* alternateMobileNumberController.text.isEmpty ? AutovalidateMode.onUserInteraction :  */_submitted==false
      ? AutovalidateMode.onUserInteraction : AutovalidateMode.always,
              /* validator: (value) {
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  addressController.text,
                  style: TextStyle(fontSize: 14, color: Theme.of(context).colorScheme.onSecondary, overflow: TextOverflow.ellipsis),
                  maxLines: 2,
                ),
              ],
            ),
              ...
              setState(() {
                if (widget.from == "location" || widget.from == "change") {
                  /*  List<dynamic> localities = detail.result.addressComponents
                      .where((entry) => entry.types.contains('locality') || entry.types.contains('administrative_area_level_1') || entry.types.contains('administrative_area_level_2'))
                      .toList()
                  ...
                  setAddressForDisplayData(
                      context,
                      "1",
                      /* localities.join("").toString() */ infoAddress.toString(),
                      detail.result.geometry!.location.lat.toString(),
                      detail.result.geometry!.location.lng.toString(),
                      detail.result.formattedAddress!.toString());
                  addSearchAddress({
                    "city": /* localities.join("").toString() */ infoAddress.toString(),
                  ...
                  states = placemarks.first.administrativeArea ?? "";

                  ///* placemarks.first.adminArea ?? "" */;
                  ...
                  areaRoadApartmentNameController.text = placemarks.first.subLocality ?? "";
                  cityController.text = /* localities.join("").toString() */ infoAddress.toString();
                  setState(() {
        ...
        : PopScope(canPop: false,
            ...
            child: Scaffold(
                ...
                body: Stack(children: [
                  ...
                  Positioned.directional(
                      textDirection: Directionality.of(context), top: height! / 99.0, start: 0, end: 0, child: placesAutoCompleteTextField()),
                ])),
          );
  }
  ...
  Future<void> getLocation() async {
    ...
    //String? location = "${placemarks.first.name},${placemarks.first.subLocality},${placemarks.first.locality},${placemarks.first.country}";
    print(
        "placemark:${placemarks[0].adminArea}--${placemarks[0].coordinates}--${placemarks[0].countryCode}--${placemarks[0].countryName}--${placemarks[0].featureName}--${placemarks[0].locality}--${placemarks[0].postalCode}--${placemarks[0].subAdminArea}--${placemarks[0].subLocality}--${placemarks[0].subThoroughfare}--${placemarks[0].thoroughfare}");
    states = /* placemarks.first.administrativeArea??"";// */ placemarks.first.adminArea ?? "";
    country = /* placemarks.first.country??"";// */ placemarks.first.countryName ?? "";
    pincode = placemarks.first.postalCode ?? "";
