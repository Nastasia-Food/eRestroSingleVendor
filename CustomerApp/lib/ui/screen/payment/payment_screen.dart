...
import 'package:erestroSingleVender/cubit/home/search/filterCubit.dart';
...
class PaymentScreenState extends State<PaymentScreen> {
  ...
  Future<void> phonepeCheckSum(String orderId) async {
    try {
      // var parameter = {orderIdKey: orderId, finalTotalKey: total().toStringAsFixed(2), mobileKey: context.read<AuthCubit>().getMobile()};
      var parameter = {
        typeKey: "cart",
      ...
      PhonePePaymentSdk.startTransaction(body, phonePeEndPointUrl!, checksums!, /* pgHeaders, apiEndPoint, */
        ...
        if (response != null) {
          ...
          if (status == 'SUCCESS') {
            ...
            print("data:--");
            //await addTransaction(/* uuid */ orderId, orderId, "Success", status, true);
            clearAll();
  ...
  Future<void> placeOrder(String? tranId) async {
    try {
      String? varientId, quantity, cartIds;
      ...
      for (int i = 0; i < cartModel.data!.length; i++) {
        ...
        cartIds = cartIds != null ? "$cartIds,${cartModel.data![i].cartId}" : cartModel.data![i].cartId;
      }
      ...
      if (response.statusCode == 200) {
        ...
        if (!error) {
          ...
          if (getdata[statusCodeKey].toString() == tockenExpireCode) {
            reLogin(context);
          }
          if (!mounted) return;
