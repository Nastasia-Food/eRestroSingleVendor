...
class DatabaseHelper {
  ...
  //insert cart data in table
  Future insertCart(String pid, String vid, String qty, String addOnId, String addOnQty, String total, String branchId, BuildContext context,
    ...
    String mcid = "${("$vid$pid$addOnId").toString().replaceAll(",", "")}"; //"$vid$pid$addOnId";
  ...
  updateCart(String pid, String vid, String qty, String addOnId, String addOnQty, String total, context, String cid, int? id) async {
    final db1 = await db;
    /* Map<String, dynamic> row = {
      DatabaseHelper._instance.QTY: qty,
      DatabaseHelper._instance.ADDONQTY: addOnQty,
    ...
    //db1!.update(CART_TABLE, row, where: "$cId = ?", whereArgs: [cid]);

    // Check if the data already exists
    List<Map<String, dynamic>> existingData =
        await db1!.query(CART_TABLE, where: "$VID = ? AND $PID = ? AND $ADDONID = ? AND $QTY = ?", whereArgs: [vid, pid, addOnId, qty]);
    // await db1!.query(CART_TABLE, where: "id = ?", whereArgs: [id]);

    if (existingData.isNotEmpty) {
      // If data exists, remove the existing row
      // await db1.delete(CART_TABLE, where: "$VID = ? AND $PID = ? AND $ADDONID = ?", whereArgs: [vid, pid, addOnId]);
      ...
      // await db1.update(CART_TABLE, where: "$VID = ? AND $PID = ? AND $ADDONID = ?", whereArgs: [vid, pid, addOnId]);
  ...
  removeCart(String vid, String pid, BuildContext context, int id) async {
    ...
    db1!.rawQuery("DELETE FROM $CART_TABLE WHERE id = ?", [id]).then((value) {});
    await getTotalCartCount(context);
  }
  ...
  Future<String?> checkCartItemExists(String pid, String vid, String addOnId) async {
    final db1 = await db;
    var result = await db1!.rawQuery("SELECT * FROM $CART_TABLE WHERE $VID = ? AND $PID = ? AND $ADDONID = ?", [vid, pid, addOnId]);
