...
const String appName = "eRestro Singlevendor Customer";
const String packageName = "com.wrteam.erestroSingleVender";
const String androidLink = 'https://play.google.com/store/apps/details?id=';
const String iosPackage = 'com.wrteam.erestroSingleVender';
const String iosLink = 'https://apps.apple.com/id';
const String iosAppId = '6459792955';
String shareAndroidAppLink = 'Delicious food at your fingertips! Order food effortlessly via  ${appName}. $androidLink$packageName';
String shareiOSAppLink = 'Delicious food at your fingertips! Order food effortlessly via  ${appName}. $iosLink$iosAppId';
...
String defaulIsoCountryAlternateCode = 'IN';
...
const String placeSearchApiKey = "PLACE_SEARCH_MAP_KEY";
...
appDataRefresh(BuildContext context) async {
  ...
  Future.delayed(Duration.zero, () async {
    await context
        .read<ProductCubit>()
        .fetchProduct(perPage, context.read<AuthCubit>().getId(), context.read<SettingsCubit>().getSettings().branchId, "", "", "", "");
  });
  Future.delayed(Duration.zero, () async {
    await context
        .read<TopRatedProductCubit>()
        .fetchTopRatedProduct(perPage, "1", context.read<AuthCubit>().getId(), context.read<SettingsCubit>().getSettings().branchId);
  });
  Future.delayed(Duration.zero, () async {
    await context
        .read<SectionsCubit>()
        .fetchSections(perPage, context.read<AuthCubit>().getId(), "", context.read<SettingsCubit>().state.settingsModel!.branchId);
  });
...
bool getStoreOpenStatus(String openTime, String closeTime) {
  ...
  if (shopOpenTimeInSeconds <= timeNowInSeconds && timeNowInSeconds <= shopCloseTimeInSeconds) {
    // OPEN;
    result = true;
  } else {
    // CLOSED;
...
setAddressForDisplayData(BuildContext context, String ifDelivery, String city, String latitude, String longitude, String address) {
  if (ifDelivery == "1") {
    context.read<CityDeliverableCubit>().fetchCityDeliverable(city.toString(), latitude, longitude);
  }
